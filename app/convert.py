...
def _option(entry: registry.StrategyEntry, values: Settings) -> AlgorithmOption:
    ...
    return AlgorithmOption(
        ...
        details=_algorithm_details(entry, values, chosen=chosen),
        chosen=chosen,
    )
...
def _algorithm_details(
    ...
    ⚠️ Отказ сборки настроек здесь **не роняет каталог и не прячется**.
    Окно выбора — это место, куда человек приходит разобраться; окно,
    упавшее вместо ответа, лишает его и разбора тоже. Поэтому причина отказа
    показывается текстом, а следом идёт правило с умолчаниями алгоритма —
    и сказано, что числа не ваши.
    ...
    """
    if not chosen:
        return entry.description(entry.defaults()).full()
    try:
        ...
        return (
            ...
            + entry.description(entry.defaults()).full()
        )
