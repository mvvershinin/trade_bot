...
#: Порог `ruff`. Замер 09.09.2026 после развязки сборки от одного торгового
#: алгоритма (миниплан `strategy-modules-switchable.md`, З2 и З5):
#: `ruff check . --output-format=json`, длина массива — 547; та же цифра
#: в строке `Found 547 errors.` обычного вывода.
#: Было 548 тем же днём после удаления веб-графика, 572 утром, 575 — 07.09.
#: Минус один — не уборка: в `app/convert.py` пропал `I001` вместе с тремя
#: строками импорта классов торгового алгоритма. ⚠️ Добавлен **один**
#: `# noqa: ANN401` в `strategies/contracts.py` — на `previous: Any` в порте
#: настроек, где `Any` вынужден контравариантностью доводов и разобран
#: в докстринге. Больше ни одна находка не подавлена.
#:
#: ⚠️ **548 с 10.09.2026, и это размен, а не рост от небрежности.** Шаг 7
#: разбора свечи (выход по обратному сигналу) выделен в
#: `engine/pipeline.py::_step_close_by_signal`: там появилось ветвление
#: порога выхода, а `process_closed_candle` и без того самая длинная функция
#: проекта. Выделение убрало из неё 20 строк (285 → 265, храповик размера
#: обновлён) и добавило **одну** находку — `PLR0913`, семь доводов у нового
#: шага, ровно та же форма, что у двух его соседей `_daily_loss_guard`
#: и `_step_open`. `PLR0917` при этом не сработал: два последних довода
#: сделаны **только именованными**, потому что `armed_before` — голое булево
#: в конце ряда однотипных объектов, и перепутанный слот здесь означает
#: снятие тейка, которого не было. Ни одна находка не подавлена.
...
#: их не было ни одной.
#: ⚠️ Развязка сборки от одного торгового алгоритма (З2, З5) это число тоже
#: не изменила, хотя по дороге поднимала до 199: сборщик настроек стал
#: возвращать порт (`StrategySettings`), и проверки, читавшие у него поля
#: алгоритма №1, получили сужение `isinstance` — не подавление, а проверка
#: того, что собран **выбранный** алгоритм.
