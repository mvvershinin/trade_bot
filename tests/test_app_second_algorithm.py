...
--------------------------
Заведён 09.09.2026, когда реестр торговых алгоритмов состоял из одной
записи. Реестр на одну запись — удобнейшее место спрятать дефект «работает
только для первого»: любая привязка к алгоритму №1 остаётся зелёной, пока
он единственный, и вылезает в первый же день сравнения двух алгоритмов —
окно показывает второй, а считает первый, **молча**.

⚠️ Настоящий второй алгоритм появился в тот же день («Тестовый скользящий»,
`ma_crossing`), и подставной **не заменяется им**. Причина: настоящий делит
с первым класс настроек — владелец счёта просил «все условия те же», — то
есть проверяет только половину механизма. Подставной сделан непохожим
нарочно, и именно он ловит сборку, знающую поля алгоритма №1.
