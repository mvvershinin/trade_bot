...
и запускается на машине владельца счёта (`available_surfaces()` 04.09.2026:
веб-график недоступен, файла библиотеки на диске нет), поэтому проверять
подстановкой вместо отрисовщика здесь нельзя: зелёный на подставном
отрисовщике ничего не говорит о том, что человек увидит на экране.
