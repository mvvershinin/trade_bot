...
def test_the_summary_says_something(entry: StrategyEntry) -> None:
    ...
    ⚠️ Здесь проверяется только наличие. Что описание **не врёт** — три
    отдельные проверки задачи З3 миниплана: исполнение утверждений, полнота
    по настройкам и граница честности.
    """
    assert entry.summary.strip(), f"у модуля {entry.id} нет описания словами"
