...
class CandleStore:
    ...
    def put_minutes(self, symbol: str, candles: Iterable[Candle], source: Source) -> WriteStats:
        ...
        **Побеждает последняя свеча минуты. Целиком — вместе с `high`, `low`
        ...
        экстремумами последнего тика, а не минуты. Сторожимый уровень
        проверяется касанием — `high >= уровень` либо `low <= уровень`, смотря
        с какой стороны до него доходят (`engine.LevelTouch`), — и по
        испорченным экстремумам он не сработает в прогоне, зато сработает
        в бою. Порча задевает **обе** стороны касания, а не только верхнюю.
        Семантика потока БКС на 31.08.2026 не подтверждена
