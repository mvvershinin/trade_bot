...
#: создание ячеек строки и подстановка чисел с перекраской. Поле, забытое
#: в одном из мест, дало бы прочерк или чужое число, и молча. Полноту таблицы
#: (все ценовые поля `Candle` показаны) и привязку подписи к своему числу
#: стережёт `tests/test_ui_panel.py`.
#:
#: Лежит здесь, а не рядом со строкой сведений (`ui/chart_panel.py`), потому
#: что вторым её потребителем была отрисовка, а та импортируется панелью —
#: обратный импорт замкнул бы круг. Второй потребитель ушёл вместе
#: с веб-графиком 09.09.2026 (решение 0056); таблица оставлена здесь,
#: потому что переезд ради переезда трогает `ui/legend.py` и три теста,
#: а места лучше не делает.
