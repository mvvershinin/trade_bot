...
⚠️ Отрисовщик сегодня один — на средствах самого Qt (решение 0056).
Встроенный веб-график удалён 09.09.2026: он не запускался ни разу, стоял в этом
списке первым и включился бы молча в день, когда рядом лёг бы файл библиотеки.
Список остался списком, потому что ТЗ §6 запасной вариант предусматривает,
а сам механизм «спросить доступность → взять первого, кто ответил да → сказать
об этом словами» и есть тот шов, ради которого заведён `ChartSurface`.
...
#: Порядок предпочтения. Пока в нём одно имя, и это не оговорка: отрисовка
#: на Qt не требует ничего, кроме PySide6, и работает на любой машине, где
#: вообще открылось окно. Второй элемент появится вместе со вторым модулем
#: рядом — и вместе с решением в `.docs/decisions/`, объясняющим, почему он
#: стоит впереди или позади этого.
ORDER: tuple[type[ChartSurface], ...] = (PainterChartSurface,)
