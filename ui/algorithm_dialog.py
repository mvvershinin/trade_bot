...
EMPTY_NOTE = (
    ...
    "части либо программа не ответила. Выбирать не из чего — закройте окно "
    "«Отменой», настройки останутся прежними. Чтобы список появился, "
    "закройте настройки и откройте их снова; если не помогло — перезапустите "
    "программу."
)
...
NOTHING_TO_CHOOSE_FROM = (
    ...
    "программа сейчас сказать не может: список не пришёл."
)
...
#: Наименьшая высота окна. Меньше нельзя: под коротким текстом окно
#: съёживается до списка с кнопками и читается как обрезанное.
WINDOW_HEIGHT = 340
...
class AlgorithmDialog(QDialog):
    ...
    def __init__(
        ...
        # Окончательная высота считается после сборки — `_fit_the_window`.
    ...
    def _fit_the_window(self) -> None:
        """Высота окна — под текст, который в нём есть, а не наоборот.

        ⚠️ Замер 09.09.2026, найдено глазами на снимке. Ярлык с переносом
        сообщает компоновщику минимум в **одну строку**, поэтому окно
        заданной высоты под длинный текст не растягивается — оно даёт
        надписям наехать друг на друга. Строка про непришедший список легла
        поверх строки про выбранную настройку, и прочитать нельзя было ни ту,
        ни другую.

        Длинный текст в этом окне бывает ровно тогда, когда что-то не так.
        То есть портилось именно то сообщение, ради которого окно открыли.

        Высота спрашивается у компоновщика по **нынешней** ширине окна:
        `heightForWidth` — единственное, что учитывает перенос строк, и
        считать его по одной ширине, а показывать по другой значило бы
        промахнуться ровно на перенос.

        ⚠️ Окно только **растёт**. Строка под списком меняется на каждый
        щелчок по нему, и окно, ужимающееся обратно, дёргалось бы под мышью.
        Зовётся отсюда, а не из конструктора: длинный текст появляется
        в `set_chosen`, то есть уже после сборки окна.
        """
        layout = self.layout()
        if layout is None:  # без компоновки считать нечего
            return
        width = max(self.width(), WINDOW_WIDTH)
        self.resize(
            width,
            max(self.height(), WINDOW_HEIGHT, layout.heightForWidth(width)),
        )
    ...
    def _show_summary(
        ...
        * список не пришёл вовсе — выхода **здесь** нет, и это говорится
        ...
        """
        self.summary.setText(self._summary_text(option, missing=missing))
        # ⚠️ После текста, а не до: высота окна считается по тому, что в нём
        # уже написано.
